  ...
  <article style="" class="card flex-col">
    <template v-if="!!product">
      <div style="position: relative">
        ...
        <div class="flex-row gap-3" style="bottom: 12px; left: 12px; align-items: center; position: absolute" @click="$emit('showSaleDescription')">
          <div v-if="!!product?.sale">
            Скидка -{{ product.sale }}&nbsp;%
          </div>
          <div v-if="!!product?.sale && !!product?.club">
            |
          </div>
          <div v-if="!!product?.club">
            Клуб -{{ product.club }}&nbsp;%
          </div>
        </div>
      </div>
      ...
      <header style="flex: 1 1 100%; align-items: stretch; justify-content: space-between; padding-left: 8px" class="flex-col gap-2">
        <NuxtLink>
          ...
          <div class="flex-row gap-2" style="margin-bottom: auto; align-items: baseline">
              ...
              <p style="font-size: 16px; text-decoration: line-through" v-if="!!product.sale || !!product.club">
